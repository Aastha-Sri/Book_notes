...
app.set('views', 'views'); // Set the views directory where your .ejs files are located
...
async function fetchAndSaveCover(isbn) {
    ...
    const fileName = isbn; // Set the filename to the ISBN.
    const imagePath = `${__dirname}/public/assets/images/covers/${fileName}.jpg`; // Construct the image path using __dirname.
    ...
    try {
        ...
        fileStream.on('finish', () => {
            console.log(`Image saved successfully: ${fileName}.jpg`);
        });
        ...
        console.error('Error fetching or saving cover image:', error); 
    }
}

export { fetchAndSaveCover };
